    ...
    pub fn from_args(matches: &ArgMatches) -> Self {
        if let Some(matches) = matches.subcommand_matches("databases") {
            let databases_opt = Databases {
                method: Method::match_method(matches.get_one::<String>("x").map(String::as_str).unwrap_or("GET")),
                notion_id: NotionIdArg(matches.get_one::<String>("id").map(String::as_str).unwrap_or("").to_string()),
                file_path: matches.get_one::<String>("file").map(String::as_str).unwrap_or("").to_string(),
            };
            ...
            let property_pages_opt = PropertyPages {
                ...
                property_id: PropertyIdArg(
                    matches.get_one::<String>("property_id").map(String::as_str).unwrap_or("").to_string(),
                ),
            };
            ...
            let blocks_opt = Blocks {
                method: Method::match_method(matches.get_one::<String>("x").map(String::as_str).unwrap_or("GET")),
                block_id: BlockIdArg(matches.get_one::<String>("id").map(String::as_str).unwrap_or("").to_string()),
                file_path: matches.get_one::<String>("file").map(String::as_str).unwrap_or("").to_string(),
            };
            ...
            let children_blocks_opt = ChildrenBlocks {
                method: Method::match_method(matches.get_one::<String>("x").map(String::as_str).unwrap_or("GET")),
                block_id: BlockIdArg(matches.get_one::<String>("id").map(String::as_str).unwrap_or("").to_string()),
                page_size: PageSizeArg(matches.get_one::<String>("page_size").map(String::as_str).unwrap_or("100").to_string()),
            };
